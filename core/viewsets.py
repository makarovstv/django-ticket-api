...
from core.permissions import IsAssigneeOrManager, IsManagerOrAdmin, IsOwnerOrManager
...
from core.tasks import notify_ticket_assigned, notify_ticket_created, notify_ticket_status_changed
...
class TicketViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = Ticket.objects.select_related("author", "assignee").prefetch_related("comments")
    ...
    def statistics(self, request):
        ...
        stats = qs.aggregate(
            total=Count("id"),
            new=Count("id", filter=__import__("django.db.models", fromlist=["Q"]).Q(status="new")),
            in_progress=Count("id", filter=__import__("django.db.models", fromlist=["Q"]).Q(status="in_progress")),
            resolved=Count("id", filter=__import__("django.db.models", fromlist=["Q"]).Q(status="resolved")),
            closed=Count("id", filter=__import__("django.db.models", fromlist=["Q"]).Q(status="closed")),
        )
...
class CommentViewSet(mixins.CreateModelMixin, mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        return Comment.objects.filter(ticket_id=self.kwargs["ticket_pk"]).select_related("author")
